...
Model.Decay.Add_NIr      <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
    ...
    Local.Mort  <- exp(-a*(2*pi*D*P/(r^2) +1))*(1 + pi*D*P*a/(r^2))
    NLocal.Mort <- exp(-a*(2*pi*D*P/(r^2) ))*(1 +pi*D*P*a/(r^2))
...
set.seed(100)
Fec.Vals   <- rlnorm(Sp,mean=0,sd=.5)  
...
write.csv(Divs_FA,"ODE_NewAdd_Y10_B.csv",quote=F,row.names=F)
write.csv(Divs_FA_SH,"ODE_NewAdd_Y10_SH_B.csv",quote=F,row.names=F)
