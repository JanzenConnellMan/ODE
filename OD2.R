...
for(ii in 1:length(SD.FEC)){
  ...
  for(jj in 1:length(Alpha.List)){
    ...
    Finals3   <- out.MDA2[length(times),2:(Sp+1)]
    Finals3   <- Finals2[Finals2> exp(-11)]
    #  Finals2   <- Finals2[!is.na(Finals2)]
    Divs_DDV2[jj] <- length(Finals2)
    Divs_DDV2SH[jj] <- ShDiv(Finals2)
...
write.csv(Divs_FA,"ODE_Neigh_E8_d1_B.csv",quote=F,row.names=F)
write.csv(Divs_DD,"ODE_Add_v5_d1_B.csv",quote=F,row.names=F)
write.csv(Divs_DD2,"ODE_Add_v5V2_d1_B.csv",quote=F,row.names=F)

write.csv(Divs_FA_SH,"ODE_Neigh_E8_d1_SH_B.csv",quote=F,row.names=F)
write.csv(Divs_DD_SH,"ODE_Add_v5_d1_SH_B.csv",quote=F,row.names=F)
write.csv(Divs_DD2_SH,"ODE_Add_v5_d1_SH_B.csv",quote=F,row.names=F)
