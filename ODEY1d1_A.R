...
Model.NearNeigh    <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
...
Model.Fixed.Add     <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
    ...
    # Seed Morality Expressions 
    Local.Mort  <- exp(-a)*exp( -(1-exp(-a))*En*P)
    NLocal.Mort <-  exp( -(1-exp(-a))*En*P)
    
    # Local seeds + mortality
    Local.seeds           <- Y*( (1-d) + P*d)*Local.Mort
    
    # Nonlocal seeds + mortality 
    NLocal.seeds          <- d*P*Y*NLocal.Mort
...
Divs_FA2 <- data.frame(matrix(NA,nrow=length(Alpha.List),ncol=length(SD.FEC)))
Divs_DD <- data.frame(matrix(NA,nrow=length(Alpha.List),ncol=length(SD.FEC)))
Divs_DD2 <- data.frame(matrix(NA,nrow=length(Alpha.List),ncol=length(SD.FEC)))


Divs_FA_SH <- data.frame(matrix(NA,nrow=length(Alpha.List),ncol=length(SD.FEC)))
Divs_FA2_SH <- data.frame(matrix(NA,nrow=length(Alpha.List),ncol=length(SD.FEC)))
Divs_DD_SH <- data.frame(matrix(NA,nrow=length(Alpha.List),ncol=length(SD.FEC)))
Divs_DD2_SH <- data.frame(matrix(NA,nrow=length(Alpha.List),ncol=length(SD.FEC)))
...
for(ii in 1:length(vlist)){
  ...
  Divs_FAV2 <- rep(NA,length(Alpha.List))
  Divs_DDV <- rep(NA,length(Alpha.List))
  Divs_DDV2<- rep(NA,length(Alpha.List))
  
  Divs_FAVSH  <- rep(NA,length(Alpha.List))
  Divs_FAV2SH  <- rep(NA,length(Alpha.List))
  Divs_DDVSH  <- rep(NA,length(Alpha.List))
  Divs_DDV2SH <- rep(NA,length(Alpha.List))
...
Divs_FA2 <- as.matrix(Divs_FA2)
Divs_DD <- as.matrix(Divs_DD)
Divs_DD2 <- as.matrix(Divs_DD2)

Divs_FA_SH <- as.matrix(Divs_FA_SH)
Divs_FA2_SH <- as.matrix(Divs_FA_SH)
Divs_DD_SH <- as.matrix(Divs_DD_SH)
Divs_DD2_SH <- as.matrix(Divs_DD2_SH)


write.csv(Divs_FA,"ODE_NonFix_Y10_A.csv",quote=F,row.names=F)
write.csv(Divs_FA2,"ODE_AddFix_Y10_A.csv",quote=F,row.names=F)
write.csv(Divs_DD,"ODE_AddExp_Y10_A.csv",quote=F,row.names=F)
write.csv(Divs_DD2,"ODE_NonExp_Y10_A.csv",quote=F,row.names=F)

write.csv(Divs_FA_SH,"ODE_NonFix_Y10_SH_A.csv",quote=F,row.names=F)
write.csv(Divs_FA2_SH,"ODE_AddFix_Y10_SH_A.csv",quote=F,row.names=F)
write.csv(Divs_DD_SH,"ODE_AddExp_Y10_SH_A.csv",quote=F,row.names=F)
write.csv(Divs_DD2_SH,"ODE_NonExp_Y10_SH_A.csv",quote=F,row.names=F)
