...
#D*pi*Neighborhood.Radius^2



Alpha.List<- seq(.1,.475,length=100)
dlist      <- seq(.3,1,length=100)

Divs_FA <- data.frame(matrix(NA,nrow=length(Alpha.List),ncol=length(dlist)))
Divs_DD <- data.frame(matrix(NA,nrow=length(Alpha.List),ncol=length(dlist)))
...
for(ii in 1:length(dlist)){
  ...
  for(jj in 1:length(Alpha.List)){
    ...
    A.Val <- Alpha.List[jj]
    #A.Min <- Alpha.List[jj]
    Alpha.Vals1 <- seq(.5-A.Val,.5+A.Val,length=Sp)
    Alpha.Vals <- -log(Alpha.Vals1)  
   ...
   # out.MDA1 <- ode(y=init, times=times, func=Model.Fixed.Area, parms=parameters,method="lsoda")
...
#write.csv(Divs_FA,"ODE_Neigh_avar.csv",quote=F,row.names=F)
write.csv(Divs_DD,"ODE_Add_v10_avar.csv",quote=F,row.names=F)
