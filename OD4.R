...
Model.Decay.Add_NIr2     <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
...
Neighborhood.Radius <- sqrt(49/(D*pi))
...
for(ii in 1:length(SD.FEC)){
 ...
 #   out.MDA3 <- ode(y=init, times=times, func=Model.Decay.Add_NIr2, parms=parameters,method="ode45")
    ...
    Divs_DDV[jj] <- length(Finals3)
    Divs_DDVSH[jj] <- ShDiv(Finals3)
...
write.csv(Divs_FA,"ODE_Neigh_E49_d1_D.csv",quote=F,row.names=F)
write.csv(Divs_DD,"ODE_Add_v75_d1_D.csv",quote=F,row.names=F)
#write.csv(Divs_DD2,"ODE_Add_v5_d1_D.csv",quote=F,row.names=F)

write.csv(Divs_FA_SH,"ODE_Neigh_E49_d1_SH_D.csv",quote=F,row.names=F)
write.csv(Divs_DD_SH,"ODE_Add_v75_d1_SH_D.csv",quote=F,row.names=F)
#write.csv(Divs_DD2_SH,"ODE_Add_v5_d1_SH_D.csv",quote=F,row.names=F)
